...
public class Bitwise {
    ...
    private static final int bitmasks2[] = {127, 191, 223, 239, 247, 251, 253, 254};
    ...
    public static boolean clear(int i, byte[] bytes) {
    	...
    	clear(i % 8, bytes[i / 8]);
        if (temp != bytes[i / 8]) {
        	return true;            //byte was changed (byte cleared from 1 to 0)
        } else {
        	return false;           //byte was not changed (byte was 0 to begin with) 
        }
    }
